...
| Path | Purpose |
|---|---|
| `src/config.py` | Project paths, constants, and date configuration |
| `src/data_ingestion.py` | Incremental GeoJSON to Parquet ingestion |
| `src/data_quality.py` | Data completeness, duplicate, grid, and validation checks |
| `src/eda.py` | Exploratory traffic-flow analysis and visualization generation |
| `src/feature_engineering.py` | Temporal features, lag features, and leakage-controlled segment statistics |
| `src/model_training.py` | Baselines and ML model training |
| `src/congestion_analysis.py` | Aggregated city-level congestion and mobility analysis |
| `src/prediction.py` | Model preprocessing and dashboard inference |

### Dashboard

| Path | Purpose |
|---|---|
| `dashboard/app.py` | Flask application and prediction endpoints |
| `dashboard/templates/index.html` | Dashboard interface |
| `dashboard/static/css/` | Dashboard styling |
| `dashboard/static/js/` | Dashboard client-side logic |

### Experiments and Validation

| Path | Purpose |
|---|---|
| `notebooks/traffic_flow_prediction_pipeline.ipynb` | End-to-end exploratory and modeling workflow |
| `tests/test_technical_audit.py` | Automated technical validation and leakage checks |
| `visualizations/` | EDA, spatial, congestion, and model diagnostic plots |

### Model Artifacts

| Path | Purpose |
|---|---|
| `models/linear_regression_ols.joblib` | Serialized Ordinary Least Squares model |
| `models/ridge_regression.joblib` | Serialized Ridge Regression model |
| `models/random_forest.joblib` | Serialized Random Forest Regressor model (top performer) |
| `models/xgboost.joblib` | Serialized XGBoost Regressor model |
| `models/segment_stats.parquet` | Out-of-sample training segment historical statistics |
| `models/model_evaluation_metrics.json` | Benchmark metrics and evaluation metadata |
...
| `docs/methodology.md` | Detailed ML methodology |
| `docs/data_quality_report.json` | Sample data quality audit output |

### Data

| Path | Purpose |
|---|---|
| `data/raw/` | Original traffic GeoJSON files |
| `data/processed/` | Partitioned Parquet datasets |

> The raw and processed traffic datasets are intentionally excluded from version control because of their size. They must be obtained/generated locally before running the full pipeline.
